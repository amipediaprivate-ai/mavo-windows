...
音频资源明细提供 FSB 转 WAV、格式转换和音频压缩。普通音频处理使用内置 FFmpeg；FSB 解码使用内置 vgmstream-cli。两套运行时都以隐藏的本地子进程工作，不会弹出命令行窗口。功能范围、保存语义和验收标准见 [`docs/audio-processing-design.md`](docs/audio-processing-design.md)。
