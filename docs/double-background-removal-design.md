...
| 蒙版柔化宽度 | 2 px | 0–8 px | 只作用于背景连通区边缘，避免全图模糊 |
...
不能全局把所有黑色或白色像素替换掉，否则主体内部的黑色、白色细节会丢失。推荐流程：

1. 从所有符合候选背景颜色的边缘像素建立种子；
2. 使用 8 邻域洪泛填充，只扩展到与背景颜色距离不超过阈值的像素；
3. 得到“与图像边缘连通的背景区”；主体内部不与边缘连通的同色区域暂时保留；
4. 对连通区边界建立 0–8 像素的柔化带，估算初始 Alpha；
