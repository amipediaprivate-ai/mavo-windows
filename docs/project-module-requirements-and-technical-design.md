# Caevir 项目模块需求与技术设计说明

> 文档状态：开发基线
> 适用平台：Caevir Windows Desktop
> 文档版本：1.0
> 更新日期：2026-08-05

## 1. 文档目标

本文档定义 Caevir“项目”模块的产品范围、业务规则、交互流程、数据模型、文件系统行为、前后端接口、异常恢复机制与验收标准。本文档作为项目模块的设计与开发基线，所有实现与测试均以本文档为准。

## 2. 背景与目标

Caevir 当前以本地资产索引为核心，资源原文件分散在用户磁盘的不同位置。项目模块在现有资产库之上增加项目级组织能力，使用户能够：
...
6. 在 Caevir 内或 Windows 文件资源管理器中管理项目子文件夹。

## 3. 核心概念

### 3.1 项目

项目由一条 Caevir 数据记录和一个本地磁盘根文件夹共同构成。项目名称与根文件夹名称始终一致。

项目包含以下核心属性：

| 属性 | 含义 |
| --- | --- |
| 项目 ID | Caevir 内部稳定标识，项目重命名或迁移后不改变 |
...
标记只在 Caevir 数据库中建立项目索引，不移动、不复制、不修改资源原文件。
...
目标项目路径必须不存在。项目名称在 Caevir 内按大小写不敏感规则保持唯一。
...
缺失目录以不可用状态显示。目录扫描不跟随 Windows 重解析点、目录联接或符号链接，所有 Caevir 写操作必须保持在项目根目录内。

### 4.6 创建项目子文件夹

用户可以通过两种方式创建子文件夹：

- 在 Caevir 项目详情中选择父目录并创建。
...
在文件资源管理器中创建、重命名或删除目录后，Caevir 通过文件监听更新目录树；重新进入项目时执行一次完整目录同步，确保离线期间的变化被识别。
...
默认目录被用户从磁盘删除后，再次向该目录添加资源时由 Caevir 重新创建。
...
目标目录存在同名文件时，Caevir 保留原扩展名并生成 `名称 (2).扩展名`、`名称 (3).扩展名`，直至得到未占用名称。任何既有文件都不会被覆盖。
...
Caevir 只自动删除或移入回收站由项目复制流程创建且仍由项目关系管理的文件。用户自行放入项目目录的文件、未知文件和自定义目录不会被清理。

## 6. 交互状态与反馈

所有文件操作在 Caevir 前端内显示进度、结果和错误，不打开命令行或控制台窗口。
...
- 持久化：现有 `caevir-index.sqlite3`，WAL 模式。
- 文件选择：`@tauri-apps/plugin-dialog`。
- 资源预览与播放：现有缩略图、`caevir-media` 协议和媒体分析链路。
...
- 所有进度、日志与错误仅显示在 Caevir 前端和后台任务面板。
...
2. 复制到目标目录中的 `.caevir-copy-{operation_id}.part`。
...
5. 用户可在 Caevir 内创建任意层级项目子文件夹。
6. 用户在文件资源管理器中创建的项目子文件夹会出现在 Caevir 目录树中。
...
15. 文件操作中断后再次启动 Caevir 能恢复到一致状态，不覆盖用户既有文件。
16. 项目相关操作全程在 Caevir 界面内反馈，Windows 运行时不出现控制台窗口。
...
用户手动放入项目目录的文件保持原样，Caevir 不覆盖、不移动、不清理。
